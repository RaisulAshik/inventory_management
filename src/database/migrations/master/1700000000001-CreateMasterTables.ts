...
export class CreateMasterTables1700000000001 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE \`admin_users\` (
        ...
        \`role\` ENUM('SUPER_ADMIN', 'ADMIN', 'SUPPORT', 'BILLING') DEFAULT 'SUPPORT',
    ...
    await queryRunner.query(`
      CREATE TABLE \`tenants\` (
        ...
        \`fiscal_year_start_month\` TINYINT DEFAULT 4,
    ...
    await queryRunner.query(`
      CREATE TABLE \`tenant_users\` (
        ...
        \`updated_at\` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
        PRIMARY KEY (\`id\`),
        UNIQUE KEY \`uk_tenant_user_email\` (\`email\`),
    ...
    await queryRunner.query(`
      CREATE TABLE \`tenant_billing_info\` (
        ...
        \`updated_at\` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
        PRIMARY KEY (\`id\`),
        UNIQUE KEY \`uk_billing_tenant\` (\`tenant_id\`),
        CONSTRAINT \`fk_billing_info_tenant\` FOREIGN KEY (\`tenant_id\`)
          REFERENCES \`tenants\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    `);
    ...
    await queryRunner.query(`
      CREATE TABLE \`subscriptions\` (
        ...
        \`tenant_id\` CHAR(36) NOT NULL,
        \`plan_id\` CHAR(36) NOT NULL,
        \`status\` ENUM('ACTIVE', 'TRIAL', 'PAST_DUE', 'SUSPENDED', 'CANCELLED', 'EXPIRED') DEFAULT 'TRIAL',
    ...
    await queryRunner.query(`
      CREATE TABLE \`global_settings\` (
        ...
        \`updated_at\` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
        PRIMARY KEY (\`id\`),
        UNIQUE KEY \`uk_global_setting_key\` (\`setting_key\`),
        INDEX \`idx_global_setting_public\` (\`is_public\`)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    `);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP TABLE IF EXISTS \`tenant_billing_info\``);
